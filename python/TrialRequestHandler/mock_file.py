...
def test_details_marketo():
    try:
        ...
        result = handler.details_marketo(id_test, os.environ['api_host'], os.environ['client_id'], os.environ['client_secret'])
...
def test_insert_dynamo():
    ...
    count_attempts = 10
    handler = TrialRequestHandler()
    resp = handler.insert_into_dynamo(test_dynamo_service(),os.environ['trial_request_table'], leadid, response, fulfilled_test, count_attempts)
...
def test_send_to_SQS():
    try:
        ...
        handler = TrialRequestHandler()
        resp = handler.send_to_SQS(test_SQS_service(),os.environ['sqs_url'],response_m)
        assert resp['ResponseMetadata']['HTTPStatusCode'] == 200
        print(resp)
